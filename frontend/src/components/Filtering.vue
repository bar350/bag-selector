...
export default {
  ...
  methods: {
    ...
    checkMechanics: function (game) {
      if (!this.filters.mechanics.shown) {
        return false
      }
    ...
    checkSuggestions: function (game) {
      if (!this.filters.suggested.shown || !this.filters.suggested.players) {
        return false
      }
    ...
    addUser: function (event) {
      ...
      // GET /someUrl
      this.$http.get('http://127.0.0.1:8000/boardgames/boardgames/?username=' + value).then(response => {
        // get body data
